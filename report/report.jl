...
### 1η Εργασία Εξαμήνου: Μετασχηματισμοί και Προβολές 
...
Της δίνεται ένα σημείο (ή πίνακας από σημεία) cp και επιστρέφει τις συντεταγμένες του σημείου/των σημείων μετά από έναν Affine μετασχηματισμό, ο οποίος αποτελείται από μία περιστροφή γύρω από άξονα $u$ ή μια μετατοπίση κατά $t$ ή έναν συνδυασμό αυτών.
...
Επιστρέφει τις προβολές των σημείων που βρίσκονται στον πίνακα _p_, σε μια φωτογραφία στον "κόσμο" **(CCS - Camera Coordinate System)** της οποίας έχουμε βάση $c_{x},c_{y},c_{z}$ και αρχή $c_{v}$, με εστιακή απόσταση _f_.
Για την επίτευξη αυτού του σκοπού καλούμε τη `system_transform()`, παράγουμε τις καινούριες συντεταγμένες (έστω $x,y,z$) στο CCS και έπειτα, ξανά από τη θεωρία:
...
# ╔═╡ 44338cb5-b26c-455b-aae3-d8c4e42542c0
